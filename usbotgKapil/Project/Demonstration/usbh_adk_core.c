...
/**
  *USBH_ADK_Init, Initialization for ADK class.
	*Parameter manufacture: manufacturer name string(max 63 chars)
  *					model: model name string (max 63 chars)
  *					description: description string (max 63 chars)
  *					version: version string (max 63 chars)
  *					uri: URI string (max 63 chars)
  *					serial: serial number string (max 63 chars)
  *Return none
  */
...
/**
  ...
  * Parameter pdev: Selected device
  * 					hdev: Selected device property
  * Return USBH_Status : Status of class request handled.
  */
static USBH_Status USBH_ADK_InterfaceInit ( USB_OTG_CORE_HANDLE *pdev, void *phost)
{
...
/**
  ...
  * Parameter pdev: Selected device
  *  					buff: receive data
  * 					len : receive data buffer length
