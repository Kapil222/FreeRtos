...
int firsttime = 0;			//When connected first time
int i = 1;
int chargershut = 0; 			//Charging Status
uint8_t * i2creaddata;
uint32_t readdata;
char binaryreaddata[9] = "0";
char* hextostring;
float voltage = 0;
int charging = 0;			//Get Charging
int idle_mincount = 0;		//Minute counter
int idle_seccount = 0; 		//Second counter
int push_button_tmr = 0;
int powerstate = 0;
int charge = 0; 		//Charge Accessory 
int statemachine = 0;
uint16_t len;
		...
		/* Push button Monitor */
			...
			/* Read Analog Voltage */
				...
				if(statemachine == ADK_IDLE ){
					...
					len = 0;					
					//Check the msg  from android device
