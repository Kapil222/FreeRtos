...
/** @defgroup USBH_CORE 
  * @brief This file handles the basic enumaration when a device is connected 
  *          to the host.
  * @{
  */ 
                               ...
                               0xff) == USBH_OK)
      {
        /* User callback for Serial number string */
				...
				if(sno_matched != 1){
					...
					//reinit_ADK();
					charge = 0;
					}else{//Serial number is same as before
						if(charge == 1){
							//application is installed on the phone
						
						}else{
							//application was not installed on the phone
							poweroff();
							chargershut = 1;
							powerstate = 0;
